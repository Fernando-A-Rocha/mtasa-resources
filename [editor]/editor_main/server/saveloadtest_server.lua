...
function createElementAttributesForSaving(xmlNode, element)
	...
	-- Newmodels hack
	local dataName = exports.newmodels_reborn:getDataNameFromType(getElementType(element))
	local customModel = tonumber(getElementData(element, dataName))
	if customModel then
		xmlNodeSetAttribute(elementNode, "model", tostring(exports.newmodels_reborn:getBaseModelIdFromCustomModelId(customModel)))
	end
	return elementNode
end
